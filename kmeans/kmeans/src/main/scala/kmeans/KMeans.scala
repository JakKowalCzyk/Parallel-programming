...
class KMeans extends KMeansInterface {
  ...
  def classify(points: Seq[Point], means: Seq[Point]): Map[Point, Seq[Point]] = {
    if(points.isEmpty) means.map(p => p -> IndexedSeq()).toMap
    else points.map(p => (findClosest(p, means), p)).groupBy(_._1).map{ case (point, value) => (point, value.map(_._2))}
  }

  def classify(points: ParSeq[Point], means: ParSeq[Point]): ParMap[Point, ParSeq[Point]] = {
    if(points.isEmpty) means.map(p => p -> IndexedSeq().par).toMap
    else points.map(p => (findClosest(p, means), p)).groupBy(_._1).map{ case (point, value) => (point, value.map(_._2))}
  }
