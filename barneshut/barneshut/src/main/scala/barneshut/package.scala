...
package object barneshut {
  ...
  extends Quad {
    ...
    def insert(b: Body): Quad = if (size > minimumSize){
      val half = size / 2
      val quart = size / 4
      val nw = Empty(centerX-quart, centerY-quart, half)
      val ne = Empty(centerX+quart, centerY-quart, half)
      val sw = Empty(centerX-quart, centerY+quart, half)
      val se = Empty(centerX+quart, centerY+quart, half)
      (bodies :+ b).foldLeft(Fork(nw, ne, sw, se))(((b, f) => b.insert(f)))
    }else{
